    ...
    try:
        ...
        if message.topic =="home/led":
            if message.payload.decode() == "LED1 ON":
                ...
                device_name = "Điều Hòa"
                state = False
            elif(message.payload.decode() == "LED4 ON"):
                device_name = "Cảnh Báo Bụi"
                state = True
            elif(message.payload.decode() == "LED4 OFF"):
                device_name = "Cảnh Báo Bụi"
                state = False
        ...
        if message.topic == "home/sensor":
            ...
            dust = data.get('randomSensor')
            count = data.get('count')
            if temperature is not None and humidity is not None and light_intensity is not None and dust is not None:
