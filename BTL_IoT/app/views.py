...
def monitor(request):
    ...
    ordering = request.GET.get('ordering', 'id_desc') 
    if ordering == 'temperature_asc':
        monitor_list = Monitor.objects.order_by('temperature')
    elif ordering == 'temperature_desc':
        monitor_list = Monitor.objects.order_by('-temperature')
    elif ordering == 'humidity_asc':
        monitor_list = Monitor.objects.order_by('humidity')
    elif ordering == 'humidity_desc':
        monitor_list = Monitor.objects.order_by('-humidity')
    elif ordering == 'light_asc':
        monitor_list = Monitor.objects.order_by('light_intensity')
    elif ordering == 'light_desc':
        monitor_list = Monitor.objects.order_by('-light_intensity')
    else:
        monitor_list = Monitor.objects.order_by('-id')
    ...
    dust_query = request.GET.get('dust_query')
    time_query = request.GET.get('time_query', '')
    ...
    if dust_query:
        monitor_list = monitor_list.filter(dust_query__icontains=dust_query)
...
def toggle_device(request):
    if request.method =='POST':
        ...
        current_time_vn = timezone.now().astimezone(vietnam_tz) 
...
def get_sensor_data(request):
    data = list(Monitor.objects.values('timestamp', 'temperature', 'humidity', 'light_intensity', 'dust'))
