...
// const char* mqtt_server = "192.168.1.2";  // Địa chỉ IP của máy tính cục bộ
const char* mqtt_server = "192.168.178.163"; 
...
void loop() {
  ...
  if (!isnan(temperature) && !isnan(humidity)) {
    ...
    Serial.println(" %");
    Serial.print(", Random Sensor: ");
    Serial.println(randomSensorValue);
    ...
    payload += ", \"randomSensor\": ";
    payload += String(randomSensorValue);
    payload += ", \"count\": ";
    payload += String(dem);  
    payload += "}";
  ...
  if ( randomSensorValue>60) {
    dem=dem+1;
    
    // Nháy đèn nếu cường độ ánh sáng thấp hơn 300
    for (int i = 0; i < 2; i++) {
      digitalWrite(LED4_PIN, HIGH);  // Bật LED1
      client.publish("home/led", "LED4 ON");
      delay(500);  // Chờ 0,5 giây
      
      digitalWrite(LED4_PIN, LOW);   // Tắt LED1
      client.publish("home/led", "LED4 OFF");
      delay(500);  // Chờ 0,5 giây
    }
  }else{
    delay(2000);
